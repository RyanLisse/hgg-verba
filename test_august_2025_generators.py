...
async def test_openai_o3():
    ...
    config = {
        ...
        "System Message": {"value": "You are an advanced AI that can reason about images and text."}
    }
    
    if os.getenv("OPENAI_API_KEY"):
        config["API Key"] = {"value": os.getenv("OPENAI_API_KEY")}
    
    query = "Explain the Pythagorean theorem and how it applies to real-world scenarios."
...
async def test_anthropic_claude4():
    ...
    config = {
        ...
        "Enable Web Search": {"value": True},  # Claude Sonnet 4 can alternate with tools
        "Temperature": {"value": 0.7},
        "Max Tokens": {"value": 4096},
        "System Message": {"value": "You are Claude Opus 4, optimized for precise instruction following."}
    }
...
async def test_gemini_deep_think():
    ...
    config = {
        ...
        "System Message": {"value": "You are Gemini Deep Think, capable of exploring multiple ideas in parallel."}
    }
    ...
    try:
        ...
        async for chunk in generator.generate_stream(config, query, context, []):
            if chunk.get("type") == "system" and chunk.get("metadata", {}).get("phase") == "deep_think_init":
...
async def test_quick_comparison():
    ...
    configs = [
        ("OpenAI GPT-4.1", OpenAIGenerator(), {
            "Model": {"value": "gpt-4.1"},
            "Temperature": {"value": 0.5},
            "System Message": {"value": "You are a helpful assistant."}
        }),
        ("Anthropic Claude 3.7 Sonnet", AnthropicGenerator(), {
            "Model": {"value": "claude-3.7-sonnet"},
            "Temperature": {"value": 0.5},
            "System Message": {"value": "You are a helpful assistant focused on coding."}
        }),
        ("Google Gemini 2.5 Flash", GeminiGenerator(), {
    ...
    for name, generator, config in configs:
        ...
        try:
            response = ""
            async for chunk in generator.generate_stream(config, query, context, []):
                if chunk.get("type") == "content" or (chunk.get("type") is None and chunk.get("message")):
