...
class Citation(BaseModel):
    ...
    source_type: SourceType = Field(
        ..., description="Type of source (document, chunk, web, etc.)"
    )
    title: str | None = Field(None, description="Title of the source document")
    ...
    confidence_score: float = Field(
        0.0, ge=0.0, le=1.0, description="Confidence in this citation's relevance"
    )
...
class ReasoningStep(BaseModel):
    ...
    confidence: float = Field(
        0.0, ge=0.0, le=1.0, description="Confidence in this step"
    )
...
class ThinkingTrace(BaseModel):
    ...
    total_thinking_time: float | None = Field(
        None, description="Total time spent thinking (in seconds)"
    )
    complexity_level: str = Field(
        "medium", description="Complexity level of the reasoning task"
    )
    final_conclusion: str = Field(
        ..., description="Final conclusion from the thinking process"
    )
...
class RAGResponse(BaseModel):
    ...
    citations: list[Citation] = Field(
        default_factory=list, description="Sources used to generate this response"
    )
    source_documents_used: int = Field(
        0, description="Number of source documents referenced"
    )

    # Reasoning and validation
    reasoning_trace: ThinkingTrace | None = Field(
        None, description="Detailed reasoning process if available"
    )
    key_insights: list[str] = Field(
        default_factory=list, description="Key insights extracted from sources"
    )
    ...
    factual_accuracy_score: float = Field(
        0.0, ge=0.0, le=1.0, description="Estimated factual accuracy"
    )
    completeness_score: float = Field(
        0.0, ge=0.0, le=1.0, description="How complete the answer is"
    )
    relevance_score: float = Field(
        0.0, ge=0.0, le=1.0, description="Relevance to the original question"
    )
    ...
    error_messages: list[str] = Field(
        default_factory=list, description="Any error messages encountered"
    )
...
class StreamingChunk(BaseModel):
    ...
    chunk_type: str = Field(
        ..., description="Type of chunk: content, reasoning, citation, metadata"
    )
    content: str = Field("", description="Content of this chunk")
    is_complete: bool = Field(
        False, description="Whether this chunk completes a section"
    )
    metadata: dict[str, Any] = Field(
...
class EnhancedRAGResponse(RAGResponse):
    ...
    extended_thinking: str | None = Field(
        None, description="Extended thinking process for complex queries"
    )
    alternative_perspectives: list[str] = Field(
        default_factory=list, description="Alternative viewpoints considered"
    )

    # Multimodal support
    image_analysis: dict[str, Any] | None = Field(
        None, description="Results from image analysis if applicable"
    )
    visual_elements: list[str] = Field(
        default_factory=list, description="Visual elements referenced"
    )

    # Tool usage tracking
    tools_used: list[str] = Field(
        default_factory=list,
        description="External tools used (web search, file search, etc.)",
    )
    tool_results: dict[str, Any] = Field(
        default_factory=dict, description="Results from tool usage"
    )

    # Quality assurance
    fact_check_status: str | None = Field(
        None, description="Status of fact-checking if performed"
    )
    cross_references: list[str] = Field(
        default_factory=list, description="Cross-references to verify information"
    )

    # User experience
    reading_time_estimate: int | None = Field(
        None, description="Estimated reading time in minutes"
    )
    complexity_rating: str = Field(
        "intermediate", description="Complexity rating for the response"
    )
...
class ValidationResult(BaseModel):
    ...
    validation_score: float = Field(
        0.0, ge=0.0, le=1.0, description="Overall validation score"
    )
...
class GeneratorConfig(BaseModel):
    ...
    enable_structured_output: bool = Field(
        True, description="Enable structured Pydantic outputs"
    )
    ...
    response_format: str = Field(
        "enhanced", description="Response format: basic, standard, enhanced"
    )
...
def create_basic_rag_response(
    answer: str, model_name: str, citations: list[Citation] = None
) -> RAGResponse:
...
def validate_response_quality(
    response: RAGResponse, min_confidence: float = 0.5
) -> ValidationResult:
    ...
    if response.factual_accuracy_score < min_confidence:
        issues.append(
            f"Factual accuracy below threshold ({response.factual_accuracy_score:.2f} < {min_confidence})"
        )
        score -= 0.3
