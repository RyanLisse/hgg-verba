...
import { Chunk } from "@/app/types/chat";
...
describe("ContentView", () => {
  ...
  test("renders all chunks", () => {
    ...
    const firstChunkContent = within(container).getByText("First chunk content");
    const secondChunkContent = within(container).getByText("Second chunk content");
  ...
  test("renders chunk sources and scores", () => {
    ...
    const firstChunkInfo = within(container).getByText("Source: document1.pdf (Score: 0.9000)");
    const secondChunkInfo = within(container).getByText("Source: document2.pdf (Score: 0.8000)");
  ...
  test("renders chunks in correct order", () => {
    ...
    const firstChunkText = within(chunkElements[0]).getByText("First chunk content");
    const secondChunkText = within(chunkElements[1]).getByText("Second chunk content");
